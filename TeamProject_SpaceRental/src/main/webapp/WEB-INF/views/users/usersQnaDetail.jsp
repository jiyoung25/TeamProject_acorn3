...
<body>
	...
	<div class="container">
		...
		<div class="row">
			<div class="col-4">	
				...
				<c:if test="${dto.qnaPrevNum ne 0 }">
					<a href="usersQnaDetail?num=${dto.qnaPrevNum }&condition=${condition}&keyword=${encodedK}" class="btn btn-outline-dark" role="button">이전글</a>
				</c:if>
				<%-- 만일 다음글(더 최신글)의 글번호가 0 이 아니라면 (다음글이 존재한다면) --%>
				<c:if test="${dto.qnaNextNum ne 0 }">
					<a href="usersQnaDetail?num=${dto.qnaNextNum }&condition=${condition}&keyword=${encodedK}" class="btn btn-outline-dark" role="button">다음글</a>
				</c:if>
				<%-- 만일 검색 키워드가 있다면 --%>
			...
			<div class="col-8" style="text-align: right;">
				...
				<c:if test="${usersCode eq 2 }">
					<a href="${pageContext.request.contextPath}/users/qnaList" class="btn btn-outline-dark" role="button">Q&A목록으로</a>
				</c:if>
		...
		<form class="comment-form insert-form" action="comment_insert"
			...
			<textarea name="content" class="form-control">${empty id ? '댓글 작성을 위해 로그인이 필요 합니다.' : '' }</textarea>
