...
<body>
	...
	<div class="container">
		...
		<div id="reservMenu">
			<button v-on:click="onReservMenu(event)" 
				...
				> 예약 요청 </button>
			<button v-on:click="onReservMenu(event)" id="wait_pay" value="2"> 결제 대기 </button>
			<button v-on:click="onReservMenu(event)" id="complete_pay" value="3"> 결제 완료 </button>
			<button v-on:click="onReservMenu(event)" id="pass_reserv" value="4"> 거절/지난 예약 </button>
		</div>
	...
	<script>
		let reservMenu = new Vue({
			el:"#reservMenu",
			methods:{
				const onReservMenu = function(event){
						...
						fetch("${pageContext.request.contextPath}/seller/reservation/getreservationlist")
			              ...
			              .then(function(data){
		                      ...
		                      for(let i=0; i<json.length; i++){
		                     	  result = "<td>"+json[i].reserv_num+"</td>"
		  						  	  +"<td>"+json[i].space_name+"</td>"
		  							  +"<td>"+json[i].users_id+"</td>"
		  							  +"<td>"+json[i].reserv_count+"</td>"
		  							  +"<td>"+json[i].reserv_date+"</td>"
		  							  +"<td>"+json[i].reserv_time+"</td>"
		  							  +"<td>"+json[i].reserv_reg+"</td>"
		  							  +"<td>"+json[i].totalMoney+"</td>"
