...
<title>Q&A detail:Ubiquitous</title>
<style>
   .content{
		...
		<div class="row">
			<div class="col-4">	
				...
				<c:if test="${dto.qnaPrevNum ne 0 }">
					<a href="sellerQnaDetail?num=${dto.qnaPrevNum }&condition=${condition}&keyword=${encodedK}" class="btn btn-outline-dark" role="button">이전글</a>
				</c:if>
				<%-- 만일 다음글(더 최신글)의 글번호가 0 이 아니라면 (다음글이 존재한다면) --%>
				<c:if test="${dto.qnaNextNum ne 0 }">
					<a href="sellerQnaDetail?num=${dto.qnaNextNum }&condition=${condition}&keyword=${encodedK}" class="btn btn-outline-dark" role="button">다음글</a>
				</c:if>
				<%-- 만일 검색 키워드가 있다면 --%>
