...
public class UsersController {
	...
	*/
	...
	public ResponseEntity<Map<String, Object>> checkToken(HttpServletRequest request) {
	    // 사용자 정보를 추출해서 Map<String, Object> 객체에 담은 후 ResponseEntity로 반환
        ...
        if (token != null && token.startsWith("Bearer ")) {
            ...
            if (jwtService.isUsable(token)) {
                id = jwtService.getSubject(token);
    ...
    public String login(@RequestBody HashMap<String, String> map, HttpServletResponse response, HttpServletRequest request){
    	System.out.println(map.get("id"));
        UsersDto loginUser= service.login(map.get("id"), map.get("password"), request.getSession(), request, response);
        String token = jwtService.createToken(map.get("id")+"", (60 * 1000 * 60));
